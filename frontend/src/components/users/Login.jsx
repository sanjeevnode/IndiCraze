...
const Login = ({ setToggleRegister }) => {
    const { setBackdrop ,isLoggedIn,setCurrentUser } = useContext(Context);
    ...
    return (
        ...
        <div className='flex flex-col px-6 py-6 gap-2'>
            ...
            <form onSubmit={ handleSubmit(formSubmit) } action="" className='flex flex-col justify-center   w-full'>
                ...
                <button className='border my-4 p-2 outline-none bg-[rgb(255,182,70)] text-white rounded-sm' type='submit'>Login</button>

            </form>
            <div className=''>
                <p className='text-gray-500'>
                    New to IndiCraze? <span onClick={ () => setToggleRegister(true) } className='text-[rgb(255,102,0)] cursor-pointer'>Create account</span>
                </p>
            </div>
        </div>
    )
